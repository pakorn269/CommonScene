...
export const DemoModeScreen: React.FC<DemoModeScreenProps> = ({ onExit }) => {
    ...
    useEffect(() => {
        ...
        const t3 = setTimeout(() => {
            ...
            const result = rankMovies(catalog, [
                PROFILE_ALICE,
                PROFILE_BOB,
                PROFILE_CHARLIE,
            ]);
    ...
    const winningMovie = recommendations[0]
        ? getMovieById(recommendations[0].movieId)
        : null;
        ...
        <View style={styles.container}>
            ...
            <View style={styles.topBar}>
                ...
                <Text style={styles.headerTitle}>
                    {step === 'winner'
                        ? 'Consensus Winner Revealed!'
                        : 'CommonScene Group Decision Flow'}
                </Text>
            </View>
                    ...
                    <View style={styles.participantsRow}>
                        ...
                        <View style={styles.profileCard}>
                            <View
                                style={[
                                    styles.avatarBox,
                                    { backgroundColor: AVATARS[0]?.bg ?? '#F97316' },
                                ]}
                            >
                            ...
                            <Text style={styles.profilePref}>
                                Loves: Family, Comedy
                            </Text>
                            <Text style={styles.profilePref}>
                                Max: 100 min • PG
                            </Text>
                            <Text style={styles.profileExclusion}>
                                Excludes: Horror
                            </Text>
                            {step === 'prefs' && (
                                <Text style={styles.readyIndicator}>
                                    ✓ Submitted
                                </Text>
                            )}
                        </View>
                        ...
                        <View style={styles.profileCard}>
                            <View
                                style={[
                                    styles.avatarBox,
                                    { backgroundColor: AVATARS[1]?.bg ?? '#10B981' },
                                ]}
                            >
                                <Text style={styles.avatarEmoji}>
                                    {AVATARS[1]?.emoji ?? '🐼'}
                        ...
                        <View style={styles.profileCard}>
                            <View
                                style={[
                                    styles.avatarBox,
                                    { backgroundColor: AVATARS[2]?.bg ?? '#F59E0B' },
                                ]}
                            >
                                <Text style={styles.avatarEmoji}>
                                    {AVATARS[2]?.emoji ?? '🦁'}
                            ...
                            <Text style={styles.profilePref}>
                                Loves: Animation, Musical
                            </Text>
                            <Text style={styles.profilePref}>
                                Max: 95 min • G
                            </Text>
                            <Text style={styles.profileExclusion}>
                                Excludes: Horror, Thriller
                            </Text>
                            {step === 'prefs' && (
                                <Text style={styles.readyIndicator}>
                                    ✓ Submitted
            ...
            {step === 'ranking' && (
                <View style={styles.rankingBox}>
                    <Text style={styles.rankingFormulaTitle}>
            ...
            {step === 'winner' && winningMovie && (
                <View style={styles.winnerCard}>
                    ...
                    <Text style={styles.winnerMeta}>
                        {winningMovie.releaseYear} • {winningMovie.runtimeMinutes}{' '}
                        min • {winningMovie.contentRating} • {winningMovie.genres.join(', ')}
                    </Text>
                    <Text style={styles.winnerSynopsis}>
                        {winningMovie.synopsis}
                    </Text>
                    <Text style={styles.winnerReason}>
                        ✨ Won unanimous group vote (Satisfied Charlie&apos;s G-rating constraint, Alice&apos;s Comedy preference, and Bob&apos;s Adventure mood).
                    </Text>
                </View>
            )}
