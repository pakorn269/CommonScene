...
export function isParticipantHardConstraintSatisfied(
  ...
  if (profile.maximumRuntimeMinutes !== null && profile.maximumRuntimeMinutes !== undefined) {
...
export function calculateParticipantSatisfaction(
  ...
  const totalPreferencesCount = profile.preferredGenres.length + profile.moods.length;
...
export function calculateGroupPreferenceCoverage(
  ...
  for (const p of profiles) {
    for (const g of p.preferredGenres) distinctRequestedGenres.add(g.toLowerCase());
    for (const m of p.moods) distinctRequestedMoods.add(m.toLowerCase());
  }

  const totalDistinct = distinctRequestedGenres.size + distinctRequestedMoods.size;
  if (totalDistinct === 0) {
 ...
 */
export function calculatePenalty(_movie: Movie, _profiles: PreferenceProfile[]): number {
 ...
 */
export function rankMovies(catalog: Movie[], profiles: PreferenceProfile[]): RankedMovie[] {
