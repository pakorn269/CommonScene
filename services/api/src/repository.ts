...
export class InMemoryRoomRepository {
  ...
  addParticipant(
    ...
    if (nameCollision) {
      throw new Error(`A participant named "${trimmedName}" is already in this room.`);
    }
  ...
  submitPreferences(
    ...
    const submittedCount = updatedParticipants.filter((p) => p.hasSubmittedPreferences).length;
  ...
  submitVote(
    ...
    const updatedVotes = [...existingVotes.filter((v) => v.participantId !== participantId), vote];
