...
export const roomRoutes: FastifyPluginAsync<RoomRoutesOptions> = async (fastify, opts) => {
  ...
  // 2. Get Room Snapshot
  // -------------------------------------------------------------------------
  fastify.get<{ Params: { roomCode: string } }>('/rooms/:roomCode', async (request, reply) => {
  ...
  fastify.post<{ Params: { roomCode: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const result = repository.addParticipant(roomCode, displayName, avatarId);
  ...
  fastify.put<{ Params: { roomCode: string; participantId: string } }>(
    ...
    async (request, reply) => {
      ...
      const bodyParse = SubmitPreferencesRequestSchema.safeParse(request.body);
      if (!bodyParse.success) {
        return reply.status(400).send({
          statusCode: 400,
          error: 'Bad Request',
      ...
      try {
        ...
        if (
          ...
          if (parsed) {
            if (preferenceData.preferredGenres.length === 0 && parsed.preferredGenres.length > 0) {
              preferenceData.preferredGenres = parsed.preferredGenres;
            }
            if (preferenceData.moods.length === 0 && parsed.moods.length > 0) {
              preferenceData.moods = parsed.moods;
            }
          }
        }

        const result = repository.submitPreferences(roomCode, participantId, preferenceData);
        ...
        const message = err instanceof Error ? err.message : 'Unable to submit preferences.';
        return reply.status(400).send({
          statusCode: 400,
          error: 'Bad Request',
          message,
  ...
  fastify.post<{ Params: { roomCode: string } }>(
    ...
    async (request, reply) => {
      ...
      if (aiEnhanced && topCandidates.length > 0) {
        await Promise.all(
          topCandidates.map(async (candidate) => {
            ...
            if (movie) {
              candidate.explanation = await defaultBedrockService.explainRecommendation(
                movie,
                candidate,
                profiles,
              );
      ...
      const topMovie = firstCandidate ? getMovieById(firstCandidate.movieId) : undefined;

      const groupSummary =
        topMovie && aiEnhanced
          ? await defaultBedrockService.generateGroupSummary(profiles, topMovie)
  ...
  fastify.post<{
    ...
    try {
      ...
      const result = repository.submitVote(roomCode, participantId, movieId, rank);
  ...
  fastify.post<{ Params: { roomCode: string } }>(
    ...
    async (request, reply) => {
      const { roomCode } = request.params;
      const winningMovieId = (request.body as { winningMovieId?: string })?.winningMovieId;
      ...
      try {
        const result = repository.finalizeWinner(roomCode, winningMovieId);
        ...
        const message = err instanceof Error ? err.message : 'Unable to finalize room.';
        return reply.status(400).send({
          statusCode: 400,
          error: 'Bad Request',
          message,
